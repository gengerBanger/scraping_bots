...
coins = {}
def scrolling_save(): # Прокрутка и сохранение страницы
    try:
...
def save_csv(path, items): # Сохранение данных в csv
...
def calculating_the_sign(time_interval): # Добавление знака к изменениям
...
def main_parser():
    ...
    for card in cards:
        ...
        try:
            title = card.find('p', {'class': 'sc-4984dd93-0 kKpPOn'}).text.upper().replace(' ', '')  # название монеты
